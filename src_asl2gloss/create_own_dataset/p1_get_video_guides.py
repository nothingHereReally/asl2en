...
for i in range(10):
    ...
    for each_video in wlasl_ds[i]['instances']:
        if not exists(f"{PROJ_ROOT /"dataset" /"wlasl" /"videos" /each_video['video_id']}.mp4"):
            raise FileExistsError(f"file does not exist {Path("dataset") /"wlasl" /"videos" /each_video['video_id']}.mp4")
        text_out2file= f"{text_out2file}mpv {Path("dataset") /"wlasl" /"videos" /each_video['video_id']}.mp4\n"
