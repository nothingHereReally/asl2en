...
from cv2 import CAP_PROP_FRAME_COUNT, VideoCapture, imwrite, COLOR_BGR2RGB, cvtColor
from mediapipe.python.solutions.holistic import Holistic
from numpy import ndarray, array, uint8, zeros


from ..lmark_essentials import drawSkeletonImg
from ..lmark_constant import IMG_SIZE, PROJ_ROOT, WLASL_VID_DIR, wlasl_READY_10, FACE_CONNECTIONS, POSE_CONNECTIONS, HAND_CONNECTIONS, WORTHY_POSE_IDX
...
if __name__=='__main__':
    ...
    write_to: str= f"{init_dir}skeleton_images/"
            ...
            for trainValTest_ins in wlasl_READY_10[tvt]:
                ...
                if exists(vidfile):
                    try:
                        ...
                        if vid.isOpened():
                            ...
                            if 0<len(images):
                                ...
                                for i in range(len(images)):
                                    ...
                                    skeleton_ann[tvt][-1]['images'].append({
                                        'file': file_id,
                        ...
                        print(f"err: {e}")
                        del e
        with open(f"{init_dir}wlasl.annotation.g10_skeleton_images.json", 'w') as f:
            dump(skeleton_ann, f)
        print("testing result( see sample details below ):")
        for i in range(len(skeleton_ann['label_id2gloss'])):
            print(f"{skeleton_ann['label_id2gloss'][  skeleton_ann['train'][i]['gloss_id']  ]}")
            print(f"video_id {skeleton_ann['train'][i]['video_id']}")
            print(f"img 1 --> {skeleton_ann['train'][i]['images'][0]}")
            print(f"img 2 --> {skeleton_ann['train'][i]['images'][1]}")
