...
from keras.src.layers import Dense, Dropout, Embedding, Layer, LayerNormalization, MultiHeadAttention
...
model: Any= load_model(f"{PROJ_ROOT /"model" /"aslvid2gloss_v61.keras"}")
TRAIN_GLOSS: int= model.output_shape[-1]   # 22 categories
QUANTITY_FRAME: int= model.input_shape[-3] # 8 frames/images
TRAIN_BATCH: int= 2
...
def trim_images(a_video_detail: dict) -> dict:
    a_video= {
        KEY_GLOSS: a_video_detail[KEY_GLOSS],
        KEY_VIDEO: a_video_detail[KEY_VIDEO],
        KEY_LMARK: a_video_detail[KEY_LMARK] #[20:-20],
    }
    if len(a_video[KEY_LMARK])<=80:
        a_video[KEY_LMARK]= a_video[KEY_LMARK][22:-20]
    elif len(a_video[KEY_LMARK])<=180:
        a_video[KEY_LMARK]= a_video[KEY_LMARK][32:-20]
    else:
        a_video[KEY_LMARK]= a_video[KEY_LMARK][42:-20]
    return a_video
        ...
        while i<len(glasl_landmark[tvt_idv]) and glasl_landmark[tvt_idv][i][KEY_GLOSS]<TRAIN_GLOSS:
            if len(batch_vid_lm)<batch:
                ...
                if len(a_video_details[KEY_LMARK])<=QUANTITY_FRAME:
                    ...
                    batch_vid_lm.append(array(get_landmark4greater(a_video_details), dtype=float32))
                batch_gloss.append(int(a_video_details[KEY_GLOSS]))
                i+= 1
            else:
