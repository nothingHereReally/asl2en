...
from ..lmark_constant import K_ID2G, K_TEST, K_TRAIN, K_VAL, PROJ_ROOT, WLASL_LANDMARK_DIR
...
def getGreaterThan_np_initHasHand(lmark_: dict) -> list:
    ...
    '''
    lmark_numpy: list= [] # be of shape(__ QUANTITY_FRAME, 518, 2 __)
...
if __name__=="__main__":
    ...
    for tvt_idv in tvt:
        tillWhat: int= 0
        while wlasl_landmark[tvt_idv][tillWhat]['gloss_id']<10:
            tillWhat+= 1
        wlasl_landmark[tvt_idv]= wlasl_landmark[tvt_idv][0:tillWhat]
    del tmp_ready
    model: Any= load_model(f"{PROJ_ROOT}model/aslvid2gloss_v19.keras")
    ...
    for tvt_idv in tvt:
        batch_vid_lm: list= []
        batch_gloss: list= []
        for vid_lm in wlasl_landmark[tvt_idv]:
            if len(batch_vid_lm)<batch:
                tmp= []
                if len(vid_lm['landmark'])<QUANTITY_FRAME:
                    tmp= getLessThan_np(vid_lm)
                elif len(vid_lm['landmark'])==QUANTITY_FRAME:
                    tmp= getEqual_np(vid_lm)
                else:
                    tmp= getGreaterThan_np_initHasHand(vid_lm)
                if len(tmp)==QUANTITY_FRAME:
                    batch_vid_lm.append(tmp)
                    batch_gloss.append(int(vid_lm['gloss_id']))
            else:
                y_pred= model.predict(
                    x=array(batch_vid_lm, dtype=float32)[:, :, 468:, :],
                    batch_size=batch
                )
